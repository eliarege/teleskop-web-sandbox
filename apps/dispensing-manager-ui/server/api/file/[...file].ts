...
router.post('/write-recipe-step', defineAuthEventHandler({
  roles: ['manage'],
  async handler(event) {
    ...
    if (status === StatusCodes.canceled || status === StatusCodes.requestCompleted) {
      if (status === StatusCodes.requestCompleted && !check) {
        ...
        await mutex.runExclusive(async () => {
          const downloaded = await tryDownloadFileFromSamba(config.reqFilePath, config.writeFilePath)
          if (downloaded) {
...
router.post('/write-dispenser-step', defineAuthEventHandler({
  roles: ['manage'],
  async handler(event) {
    const body = await readBody(event)
    const contentString = `${body.content.join(',')},${EOL.windows}`
    ...
    await mutex.runExclusive(async () => {
      const downloaded = await tryDownloadFileFromSamba(reqFilePath, config.writeFilePath)
      if (downloaded) {
        await fs.appendFile(config.writeFilePath, contentString, 'utf8')
      } else {
        await fs.writeFile(config.writeFilePath, contentString, 'utf8')
...
async function tryDownloadFileFromSamba(reqFilePath: string, writeFilePath: string) {
  try {
