...
async function updateAlarmVisibility(alarmNo: number, showOnScreen: boolean) {
  await $fetch('/api/alarms', {
    method: 'PUT',
    body: {
      machineId: props.activeMachine,
...
<template>
  ...
  >
    <div class="border-1 px-3 rounded bg-gray-100 w-full flex justify-between items-center">
      <span class="font-bold">{{ alarm.alarmNo }} {{ alarm.alarmName }}</span>
      <q-toggle
        v-model="alarm.showOnScreen"
        @update:model-value="(v) => updateAlarmVisibility(alarm.alarmNo, v)"
      />
    </div>
  </div>
</template>
