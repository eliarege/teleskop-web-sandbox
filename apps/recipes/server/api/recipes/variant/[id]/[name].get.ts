...
export default defineEventHandler(async (event) => {
  const { id, name } = getRouterParams(event)
  const { machineId } = getQuery(event)
  const decodedName = decodeURIComponent(name)

  const rawRecipeSteps = await dmsDB('RECIPE_VARIANT_STEP as s')
    .select({
    ...
    .leftJoin('PROGRAM_HEADER as p', (builder) => {
      builder
        ...
        .andOn('p.machine_id', 's.machine_id')
    })
    .leftJoin('RECIPE_VARIANT_MATERIAL as m', function () {
      this.on('s.recipe_id', '=', 'm.recipe_id')
        ...
        .andOn('s.type', '=', 'm.type')
        .andOn('s.variant_name', '=', 'm.variant_name')
    })
