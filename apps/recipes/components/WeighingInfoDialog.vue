...
const columnsAuto: (QTableColumn<WeighingAuto>)[] = [
  ...
  { name: 'dispenserId', label: t('dispenserFields.ID'), field: 'dispenserId', format: val => (val === -1) ? '____' : val },
  ...
  { name: 'actualAmount', label: t('weighingFields.ActualAmount'), field: 'actualAmount', format: val => (val === -1) ? '____' : val },
...
const columnsManual: (QTableColumn<WeighingManual>[]) = [
  ...
  { name: 'recipeType', label: t('RecipeType'), field: 'recipeType', format: val => t(`recipeTypes.${val}`) },
  { name: 'materialCode', label: t('materialFields.Code'), field: 'materialCode' },
  { name: 'materialName', label: t('materialFields.Name'), field: 'materialName' },
  { name: 'actualAmount', label: t('weighingFields.RecipeAmount'), field: 'actualAmount', format: val => (val === -1) ? '____' : val },
  { name: 'status', label: t('Status'), field: 'status' },
  {
    name: 'requestTime',
    label: t('weighingFields.RequestTime'),
