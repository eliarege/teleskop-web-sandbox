...
async function getPrograms() {
  ...
  for (const program of programs.value) {
    let chemSteps = program.steps.filter(step => step.type === RecipeType.CHEM).length
    while (chemSteps < program.chemRequests) {
      program.steps.push({ type: RecipeType.CHEM, stepNo: chemSteps, materials: [] })
      chemSteps++
    }

    let dyeSteps = program.steps.filter(step => step.type === RecipeType.DYE).length
    while (dyeSteps < program.dyeRequests) {
      program.steps.push({ type: RecipeType.DYE, stepNo: dyeSteps, materials: [] })
      dyeSteps++
    }

    let saltSteps = program.steps.filter(step => step.type === RecipeType.SALT).length
    while (saltSteps < program.saltRequests) {
      program.steps.push({ type: RecipeType.SALT, stepNo: saltSteps, materials: [] })
      saltSteps++
    }
  }
...
async function onProgramSelect(program: ProgramHeader | undefined) {
  if (program) {
    ...
    for (let i = 0; i < program.chemRequests; i++)
      recipeStep.steps.push({ type: RecipeType.CHEM, stepNo: i, materials: [] })
    for (let i = 0; i < program.dyeRequests; i++)
      recipeStep.steps.push({ type: RecipeType.DYE, stepNo: i, materials: [] })
    for (let i = 0; i < program.saltRequests; i++)
      recipeStep.steps.push({ type: RecipeType.SALT, stepNo: i, materials: [] })
...
async function onSave() {
  if (hasChanges.value) {
    const hasEmptyMaterials = editedPrograms.value?.some(program =>
      program.steps.some(step => step.materials.length === 0),
    )
    ...
    if (hasEmptyMaterials) {
      q.dialog({
        ...
        componentProps: {
          bodyText: t('confirmationDialogBody.EmptyRecipeSteps'),
          confirmBtn: {
            label: t('Confirm'),
            color: 'positive',
            icon: 'done',
        ...
        if (props.isNew) {
          await $fetch(`/api/recipes/master/${props.recipeId}`, {
            method: 'PUT',
            body: { recipe: { recipeId: props.recipeId, recipeName: props.recipeName } },
          })
        }
        await $fetch(`/api/recipes/master/${props.recipeId}`, {
          method: 'POST',
          body: { recipeName: recipeName.value, steps: editedPrograms.value },
        })
        onDialogOK(true)
      })
    } else {
