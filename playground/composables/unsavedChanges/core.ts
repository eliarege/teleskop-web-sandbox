...
export interface UnsavedChangesCoreReturn<T> {
  ...
  markSaved: () => void
  createUnsavedChangesGuard: (withSave: boolean, message?: string) => ReturnType<ReturnType<typeof useQuasar>['dialog']> | null
...
export function createUnsavedChangesCore<T>(
  ...
  const createUnsavedChangesGuard = (withSave: boolean, message?: string) => {
    ...
    const saveAction = { label: dialogOptions?.saveLabel || 'Save', value: 'confirm' as const }
    const discardAction = { label: dialogOptions?.discardLabel || 'Discard', value: 'discard' as const }
    ...
    return new Promise<boolean>((resolve) => {
      $q.dialog({
        ...
        componentProps: {
          ...
          okActions: [{ label: 'Retry', value: 'ok' }],
          cancelLabel: 'Cancel',
          dialogProps: {
            transitionShow: 'fade',
            transitionHide: 'fade',
